...
@st.cache_data
def load_filters():
    months = con.execute("SELECT DISTINCT Month FROM df WHERE Month IS NOT NULL").df()["Month"].tolist()
    countries = con.execute("SELECT DISTINCT Country_New FROM df WHERE Country_New IS NOT NULL").df()["Country_New"].tolist()
