...
# 1. Initialize native wide layout parameters
st.set_page_config(layout="wide")

# 2. Complete CSS Cleanse to Overwrite Native Gray Wrappers Permanently
st.markdown(
    """
    <style>
    /* Force main app canvas, blocks, and vertical headers to pure slate white */
    .stApp, [data-testid="stMainSpaceContainer"], [data-testid="stVerticalBlock"] {
        background-color: #ffffff !important;
    }
    
    /* Clean, floating premium client metric card boxes */
    [data-testid="stMetricContainer"], .st-emotion-cache-12w0qpk, .st-emotion-cache-7ym5gk {
        background-color: #ffffff !important;
    ...
    /* Custom KPI Labels & Value Typography overrides */
    .client-kpi-label {
        font-size: 13px;
        font-weight: 600;
        color: #64748b;
    ...
    /* Give the dashboard title clean spacing and a premium weight */
    h1 {
...
# -----------------------------
# 🎛️ SIDEBAR CONTROL FILTER MATRIX PANEL (Clears Main Canvas Clutter)
# -----------------------------
with st.sidebar:
    st.markdown("### 🎛️ Target Segment Filters")
    st.write("Refine your target audience data settings below:")
    selected_countries = st.multiselect("🌍 Country", countries, key="sidebar_country_input")
    selected_months = st.multiselect("📅 Month", months, key="sidebar_month_input")
    segment = st.selectbox("👤 Segment", ["Total", "Male", "Female"], key="sidebar_segment_input")
    brand_options = list(brand_map.keys())
    selected_brand = st.selectbox("🏢 Brand", brand_options, key="sidebar_brand_input")
...
with tab1:
    ...
    # Grid Layout with solid floating white background modular containers
    ...
    with col1:
        ...
        with st.container(border=True):
            ...
            st.altair_chart(create_sparkline_chart(df_sp1, '#0ea5e9'), use_container_width=True)
    ...
    with col2:
        ...
        with st.container(border=True):
            ...
            st.altair_chart(create_sparkline_chart(df_sp2, '#10b981'), use_container_width=True)
    ...
    with col3:
        ...
        with st.container(border=True):
            ...
            st.altair_chart(create_sparkline_chart(df_sp3, '#8b5cf6'), use_container_width=True)
    ...
    with col4:
        ...
        with st.container(border=True):
            ...
            st.altair_chart(create_sparkline_chart(df_sp4, '#f43f5e'), use_container_width=True)
    ...
    with st.container(border=True):
        ...
        attr_chart = alt.Chart(df_matrix).mark_bar(
            ...
            color=alt.Color("Agreement Score (%):Q", scale=alt.Scale(scheme="indigo"), legend=None),
...
with tab2:
    ...
    with st.container(border=True):
        ...
        st.caption("Note: Trends visualize data based on filters toggled in the left sidebar dashboard control module.")
        
        graph_where = build_where(selected_months, selected_countries, segment)
        ...
        if not df_trends.empty and df_trends['val'].sum() > 0:
            ...
            st.warning("⚠️ No active dataset parameters match the selected sidebar filters profile configurations.")
...
with tab3:
    ...
    with st.container(border=True):
        ...
        if user_query:
            st.info("✅ High-level summary metrics compiled using active dataset filters from the sidebar panel.")
